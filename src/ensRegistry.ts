...
export function handleNewOwnerRegistry(event: NewOwnerEvent): void {
  let subnode = makeSubnode(event)
...
export function handleNewResolverRegistry(event: NewResolverEvent): void {
  let node = event.params.node.toHexString()
...
export function handleNewTTLRegistry(event: NewTTLEvent): void {
...
export function handleTransferRegistry(event: TransferEvent): void {
  let domain = getDomain(event.params.node.toHexString())!
  if(domain.isMigrated == false){
